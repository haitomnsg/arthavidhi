...
<?php $__env->startSection('title', 'CRM Contacts'); ?>
...
<div x-data="contactManager()" class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
        <form method="GET" action="<?php echo e(route('crm.contacts')); ?>" class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1">
            ...
            <select name="type" class="px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white">
                ...
                <option value="lead" <?php echo e(request('type') === 'lead' ? 'selected' : ''); ?>>Lead</option>
                <option value="prospect" <?php echo e(request('type') === 'prospect' ? 'selected' : ''); ?>>Prospect</option>
                <option value="customer" <?php echo e(request('type') === 'customer' ? 'selected' : ''); ?>>Customer</option>
                <option value="partner" <?php echo e(request('type') === 'partner' ? 'selected' : ''); ?>>Partner</option>
            </select>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
                                            <?php if($contact->email): ?>
                                                <span><i class="fas fa-envelope mr-1"></i><?php echo e($contact->email); ?></span>
                                            <?php endif; ?>
                                            <?php if($contact->phone): ?>
                                                <span><i class="fas fa-phone mr-1"></i><?php echo e($contact->phone); ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                                ...
                                <?php if($contact->designation): ?>
                                    <br><span class="text-xs text-gray-400"><?php echo e($contact->designation); ?></span>
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 text-xs rounded-full font-medium
                                    <?php echo e($contact->status === 'active'
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300"><?php echo e($contact->deals_count); ?></td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300"><?php echo e($contact->tasks_count); ?></td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    <button @click="openViewModal(<?php echo e($contact->toJson()); ?>)" class="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="View Details">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                    <button @click="openEditModal(<?php echo e($contact->toJson()); ?>)" class="p-2 text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 rounded-lg transition-colors" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <form method="POST" action="<?php echo e(route('crm.contacts.destroy', $contact)); ?>" onsubmit="return confirm('Delete this contact?')">
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" @click.stop>
                ...
                <form :action="editMode ? '/crm/contacts/' + form.id : '<?php echo e(route('crm.contacts.store')); ?>'" method="POST">
                    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\billing\resources\views\crm\contacts.blade.php ENDPATH**/ ?>