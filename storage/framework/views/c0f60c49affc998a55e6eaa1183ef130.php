...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex space-x-3">
            <a href="<?php echo e(route('salaries.index')); ?>" class="inline-flex items-center px-4 py-2 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                <i class="fas fa-arrow-left mr-2"></i> Back to Salaries
            </a>
            <a href="<?php echo e(route('salaries.advance.create')); ?>" class="inline-flex items-center px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-800 dark:text-white">Rs. <?php echo e(number_format($totalAdvances, 0)); ?></p>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
    ...
    <!-- Filters -->
    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
        <form action="<?php echo e(route('salaries.advances')); ?>" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
            ...
            <div>
                <select name="status" class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-primary-500">
                    ...
                    <option value="partially_deducted" <?php echo e(request('status') === 'partially_deducted' ? 'selected' : ''); ?>>Partially Deducted</option>
                    <option value="fully_deducted" <?php echo e(request('status') === 'fully_deducted' ? 'selected' : ''); ?>>Fully Deducted</option>
                </select>
            </div>
            <div class="flex gap-2">
                ...
                <a href="<?php echo e(route('salaries.advances')); ?>" class="px-4 py-2 text-gray-500 hover:text-gray-700 dark:text-gray-300">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-3">
                                <?php if($advance->employee->photo): ?>
                                <img src="<?php echo e(asset('storage/' . $advance->employee->photo)); ?>" class="w-8 h-8 rounded-full object-cover">
                                <?php else: ?>
                                <div class="w-8 h-8 bg-primary-100 dark:bg-primary-900/30 rounded-full flex items-center justify-center text-primary-600 text-xs font-bold">
                                    <?php echo e(strtoupper(substr($advance->employee->name, 0, 1))); ?>

                                </div>
                                <?php endif; ?>
                                <div>
                                    <p class="font-medium text-gray-800 dark:text-white"><?php echo e($advance->employee->name); ?></p>
                                    <p class="text-xs text-gray-500"><?php echo e($advance->employee->departmentModel->name ?? ''); ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-gray-600 dark:text-gray-400"><?php echo e($advance->advance_date->format('M d, Y')); ?></td>
                        <td class="px-6 py-4 text-right font-medium text-gray-800 dark:text-white">Rs. <?php echo e(number_format($advance->amount, 0)); ?></td>
                        <td class="px-6 py-4 text-right <?php echo e($advance->remaining_amount > 0 ? 'text-red-600 font-medium' : 'text-green-600'); ?>">
                            Rs. <?php echo e(number_format($advance->remaining_amount, 0)); ?>

                        </td>
                        <td class="px-6 py-4 text-gray-600 dark:text-gray-400 capitalize"><?php echo e(str_replace('_', ' ', $advance->payment_method ?? '-')); ?></td>
                        <td class="px-6 py-4">
                            <?php if($advance->status === 'fully_deducted'): ?>
                            ...
                            <?php elseif($advance->status === 'partially_deducted'): ?>
                        ...
                        <td class="px-6 py-4 text-gray-500 dark:text-gray-400 text-sm max-w-[200px] truncate"><?php echo e($advance->reason ?? '-'); ?></td>
                        <td class="px-6 py-4 text-right">
                            <form action="<?php echo e(route('salaries.advance.destroy', $advance)); ?>" method="POST" class="inline"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\billing\resources\views\salaries\advances.blade.php ENDPATH**/ ?>