...
<div x-data="dealManager()" class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-6 gap-3">
        ...
        @foreach($pipelineSummary as $stage => $data)
            <a href="{{ route('crm.deals', ['stage' => $stage]) }}"
               class="p-4 rounded-xl border transition-all hover:shadow-md
                ...
                <p class="text-xs font-semibold uppercase tracking-wide text-{{ $stageColors[$stage] }}-600 dark:text-{{ $stageColors[$stage] }}-400">{{ $stage }}</p>
                <p class="text-xl font-bold text-gray-800 dark:text-white mt-1">{{ $data['count'] }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400">Rs. {{ number_format($data['value'], 0) }}</p>
            </a>
        @endforeach
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
        <form method="GET" action="{{ route('crm.deals') }}" class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search deals..."
            ...
            @if(request()->hasAny(['search', 'stage']))
                <a href="{{ route('crm.deals') }}" class="px-4 py-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 rounded-lg transition-colors">Clear</a>
            @endif
        </form>
    </div>
                    ...
                    @forelse($deals as $deal)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                            <td class="px-6 py-4">
                                ...
                                @if($deal->description)
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate max-w-xs">{{ $deal->description }}</p>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    <div class="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center flex-shrink-0">
                                        <span class="text-primary-600 font-semibold text-xs">{{ $deal->contact ? strtoupper(substr($deal->contact->name, 0, 2)) : '?' }}</span>
                                    </div>
                                    <span class="text-sm text-gray-700 dark:text-gray-300">{{ $deal->contact->name ?? 'N/A' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="font-semibold text-green-600 dark:text-green-400">Rs. {{ number_format($deal->value, 2) }}</span>
                            </td>
                            <td class="px-6 py-4">
                                @php $color = $stageColors[$deal->stage] ?? 'gray'; @endphp
                                <span class="px-2 py-1 text-xs rounded-full font-medium bg-{{ $color }}-100 text-{{ $color }}-700 dark:bg-{{ $color }}-900/30 dark:text-{{ $color }}-400">
                                    {{ ucfirst($deal->stage) }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 text-xs rounded-full font-medium
                                    @if($deal->priority === 'high') bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400
                                    @elseif($deal->priority === 'medium') bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                                @if($deal->closed_date)
                                    <span class="{{ $deal->stage === 'won' ? 'text-green-600' : 'text-red-600' }}">
                                        {{ $deal->closed_date->format('M d, Y') }}
                                    </span>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    <button @click="openEditModal({{ $deal->toJson() }})" class="p-2 text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 rounded-lg transition-colors" title="Edit">
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" @click.stop>
                ...
                <form :action="editMode ? '/crm/deals/' + form.id : '{{ route('crm.deals.store') }}'" method="POST">
                    @csrf
