...
<div class="space-y-6" x-data="{ showModal: false, editMode: false, category: { id: null, name: '', description: '' } }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-500">Organize your products into categories</p>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            <div class="flex items-start justify-between">
                ...
                <div class="flex items-center space-x-1">
                    <button @click="showModal = true; editMode = true; category = { id: {{ $category->id }}, name: '{{ $category->name }}', description: '{{ $category->description }}' }" 
                            class="p-2 text-gray-500 hover:text-blue-500 hover:bg-gray-100 rounded-lg">
                    ...
                    <form action="{{ route('categories.destroy', $category) }}" method="POST" class="inline"
                        ...
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="p-2 text-gray-500 hover:text-red-500 hover:bg-gray-100 rounded-lg">
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                ...
                <h3 class="text-lg font-semibold text-gray-800 mb-4" x-text="editMode ? 'Edit Category' : 'Add Category'"></h3>
                
                <form :action="editMode ? '{{ url('categories') }}/' + category.id : '{{ route('categories.store') }}'" method="POST" class="space-y-4">
                    @csrf
