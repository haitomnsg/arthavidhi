...
@section('title', 'Salary - ' . $salary->employee->name)
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-gray-500 dark:text-gray-400">{{ $salary->month_name }} {{ $salary->year }}</p>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <div class="flex items-center space-x-4">
                    ...
                    @endif
                    <div>
                        <h2 class="text-xl font-bold text-gray-800 dark:text-white">{{ $salary->employee->name }}</h2>
                        <p class="text-gray-500 dark:text-gray-400">{{ $salary->employee->designation ?? $salary->employee->position }}</p>
                        <div class="flex items-center gap-4 mt-1 text-sm text-gray-400">
                            @if($salary->employee->departmentModel)
                            <span><i class="fas fa-building mr-1"></i> {{ $salary->employee->departmentModel->name }}</span>
                            @endif
                            @if($salary->employee->employee_id)
                            <span><i class="fas fa-id-badge mr-1"></i> {{ $salary->employee->employee_id }}</span>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
                    <h3 class="font-semibold text-gray-800 dark:text-white">Payslip for {{ $salary->month_name }} {{ $salary->year }}</h3>
                </div>
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                                <div class="flex justify-between">
                                    ...
                                    <span class="font-medium text-gray-800 dark:text-white">Rs. {{ number_format($salary->basic_salary, 2) }}</span>
                                </div>
                                @if($salary->bonus > 0)
                                <div class="flex justify-between">
                                    <span class="text-gray-600 dark:text-gray-400">Bonus</span>
                                    <span class="font-medium text-green-600">Rs. {{ number_format($salary->bonus, 2) }}</span>
                                </div>
                                @endif
                                <hr class="border-gray-200 dark:border-gray-600">
                                <div class="flex justify-between font-semibold">
                                    <span class="text-gray-700 dark:text-gray-300">Gross Salary</span>
                                    <span class="text-gray-800 dark:text-white">Rs. {{ number_format($salary->gross_salary, 2) }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                                @if($salary->ssf_employee > 0)
                                <div class="flex justify-between">
                                    <span class="text-gray-600 dark:text-gray-400">SSF (Employee 1%)</span>
                                    <span class="font-medium text-red-500">Rs. {{ number_format($salary->ssf_employee, 2) }}</span>
                                </div>
                                @endif
                                @if($salary->tds > 0)
                                <div class="flex justify-between">
                                    <span class="text-gray-600 dark:text-gray-400">TDS</span>
                                    <span class="font-medium text-red-500">Rs. {{ number_format($salary->tds, 2) }}</span>
                                </div>
                                @endif
                                @if($salary->advance_deduction > 0)
                                <div class="flex justify-between">
                                    <span class="text-gray-600 dark:text-gray-400">Advance Deduction</span>
                                    <span class="font-medium text-red-500">Rs. {{ number_format($salary->advance_deduction, 2) }}</span>
                                </div>
                                @endif
                                ...
                                <div class="flex justify-between">
                                    <span class="text-gray-600 dark:text-gray-400">
                                        ...
                                        <span class="text-xs">({{ $salary->deduction_reason }})</span>
                                        @endif
                                    </span>
                                    <span class="font-medium text-red-500">Rs. {{ number_format($salary->deductions, 2) }}</span>
                                </div>
                                @endif
                                <hr class="border-gray-200 dark:border-gray-600">
                                <div class="flex justify-between font-semibold">
                                    <span class="text-gray-700 dark:text-gray-300">Total Deductions</span>
                                    <span class="text-red-600">Rs. {{ number_format($salary->total_deductions, 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 p-4 bg-primary-50 dark:bg-primary-900/20 rounded-xl border border-primary-100 dark:border-primary-800">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-2xl font-bold text-primary-600">Rs. {{ number_format($salary->net_salary, 2) }}</span>
                        </div>
                    </div>

                    @if($salary->ssf_employer > 0)
                    <div class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-sm">
                        <span class="text-blue-700 dark:text-blue-400"><i class="fas fa-info-circle mr-1"></i> SSF Employer Contribution (2%): Rs. {{ number_format($salary->ssf_employer, 2) }}</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <p class="text-gray-600 dark:text-gray-400 whitespace-pre-line">{{ $salary->notes }}</p>
            </div>
            @endif
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium capitalize {{ $salary->status === 'paid' ? 'text-green-600' : ($salary->status === 'hold' ? 'text-orange-600' : 'text-yellow-600') }}">
                            {{ $salary->status }}
                        </span>
                    </div>
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-800 dark:text-white">{{ $salary->payment_date->format('M d, Y') }}</span>
                    </div>
                    @endif
                    @if($salary->payment_method)
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Method</span>
                        <span class="font-medium text-gray-800 dark:text-white capitalize">{{ str_replace('_', ' ', $salary->payment_method) }}</span>
                    </div>
                    @endif
                    @if($salary->payment_reference)
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Reference</span>
                        <span class="font-medium text-gray-800 dark:text-white">{{ $salary->payment_reference }}</span>
                    </div>
                    @endif
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Created</span>
                        <span class="font-medium text-gray-800 dark:text-white">{{ $salary->created_at->format('M d, Y') }}</span>
                    </div>
                </div>

                @if($salary->status !== 'paid')
                <form action="{{ route('salaries.mark-paid', $salary) }}" method="POST" class="mt-6 space-y-3 border-t border-gray-100 dark:border-gray-700 pt-4">
                    @csrf
                    <div>
