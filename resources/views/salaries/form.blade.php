...
@section('title', isset($salary) ? 'Edit Salary' : 'Add Salary')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
            <a href="{{ route('salaries.index') }}" class="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                <i class="fas fa-arrow-left text-gray-600 dark:text-gray-400"></i>
            </a>
            <div>
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white">{{ isset($salary) ? 'Edit Salary Record' : 'Add Salary Record' }}</h1>
                <p class="text-gray-500 dark:text-gray-400">{{ date('F', mktime(0,0,0,$month,1)) }} {{ $year }}</p>
            </div>
        </div>
    </div>

    <form action="{{ isset($salary) ? route('salaries.update', $salary) : route('salaries.store') }}" method="POST" x-data="salaryForm()">
        @csrf
        @if(isset($salary))
        @method('PUT')
        @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="md:col-span-1">
                            ...
                            <select name="employee_id" x-model="employeeId" @change="loadEmployeeSalary()" required
                                ...
                                <option value="{{ $emp->id }}" 
                                        ...
                                        data-advance="{{ ($pendingAdvances[$emp->id] ?? collect())->sum('remaining_amount') }}"
                                        {{ (isset($salary) ? $salary->employee_id : old('employee_id')) == $emp->id ? 'selected' : '' }}>
                                    {{ $emp->name }} {{ $emp->departmentModel ? '(' . $emp->departmentModel->name . ')' : '' }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="month" x-model="month" required
                                    class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-primary-500">
                                @for($m = 1; $m <= 12; $m++)
                                <option value="{{ $m }}" {{ (isset($salary) ? $salary->month : $month) == $m ? 'selected' : '' }}>{{ date('F', mktime(0,0,0,$m,1)) }}</option>
                                @endfor
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="year" x-model="year" required
                                ...
                                <option value="{{ $y }}" {{ (isset($salary) ? $salary->year : $year) == $y ? 'selected' : '' }}>{{ $y }}</option>
                                @endfor
                            </select>
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="deduction_reason" value="{{ isset($salary) ? $salary->deduction_reason : old('deduction_reason') }}"
                                   class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-primary-500"
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="status" required
                                    class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-primary-500">
                                <option value="pending" {{ (isset($salary) ? $salary->status : 'pending') === 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="paid" {{ (isset($salary) && $salary->status === 'paid') ? 'selected' : '' }}>Paid</option>
                                <option value="hold" {{ (isset($salary) && $salary->status === 'hold') ? 'selected' : '' }}>On Hold</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Payment Date</label>
                            <input type="date" name="payment_date" value="{{ isset($salary) && $salary->payment_date ? $salary->payment_date->format('Y-m-d') : old('payment_date') }}"
                                   class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-primary-500">
                        </div>
                        <div>
                            ...
                            <select name="payment_method"
                                ...
                                <option value="bank_transfer" {{ (isset($salary) && $salary->payment_method === 'bank_transfer') ? 'selected' : '' }}>Bank Transfer</option>
                                <option value="cash" {{ (isset($salary) && $salary->payment_method === 'cash') ? 'selected' : '' }}>Cash</option>
                                <option value="cheque" {{ (isset($salary) && $salary->payment_method === 'cheque') ? 'selected' : '' }}>Cheque</option>
                                <option value="esewa" {{ (isset($salary) && $salary->payment_method === 'esewa') ? 'selected' : '' }}>eSewa</option>
                                <option value="khalti" {{ (isset($salary) && $salary->payment_method === 'khalti') ? 'selected' : '' }}>Khalti</option>
                            </select>
                        </div>
                    </div>
                    <div class="mt-4">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Payment Reference</label>
                        <input type="text" name="payment_reference" value="{{ isset($salary) ? $salary->payment_reference : old('payment_reference') }}"
                               class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-primary-500"
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="notes" rows="2"
                                  class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-primary-500"
                                  placeholder="Any additional notes...">{{ isset($salary) ? $salary->notes : old('notes') }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 sticky top-6">
                    ...
                    <div class="mt-6 space-y-3">
                        <button type="submit" class="w-full px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors">
                            <i class="fas fa-save mr-2"></i> {{ isset($salary) ? 'Update Salary' : 'Save Salary' }}
                        </button>
                        <a href="{{ route('salaries.index') }}" class="block w-full px-4 py-2 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-center">
                            Cancel
                        </a>
                    </div>
                </div>
...
function salaryForm() {
    return {
        employeeId: '{{ isset($salary) ? $salary->employee_id : old("employee_id", "") }}',
        month: '{{ isset($salary) ? $salary->month : $month }}',
        year: '{{ isset($salary) ? $salary->year : $year }}',
        basicSalary: {{ isset($salary) ? $salary->basic_salary : (old('basic_salary') ?: 0) }},
        bonus: {{ isset($salary) ? $salary->bonus : (old('bonus') ?: 0) }},
        deductions: {{ isset($salary) ? $salary->deductions : (old('deductions') ?: 0) }},
        advanceDeduction: {{ isset($salary) ? $salary->advance_deduction : (old('advance_deduction') ?: 0) }},
        ssfEmployee: {{ isset($salary) ? $salary->ssf_employee : (old('ssf_employee') ?: 0) }},
        ssfEmployer: {{ isset($salary) ? $salary->ssf_employer : (old('ssf_employer') ?: 0) }},
        tds: {{ isset($salary) ? $salary->tds : (old('tds') ?: 0) }},
        pendingAdvance: 0,
