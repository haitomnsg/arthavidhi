...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center space-x-4">
            <a href="{{ route('products.index') }}" class="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                    ...
                    <div class="flex-1 space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-xl font-medium text-gray-700 dark:text-gray-300">Rs. {{ number_format($product->purchase_price, 2) }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg text-center">
                        ...
                        <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ $product->min_stock_level }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ $product->unit }}</p>
                    </div>
                    <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg text-center">
                        <p class="text-sm text-gray-500 dark:text-gray-400">Stock Value</p>
                        <p class="text-2xl font-bold text-gray-800 dark:text-white">Rs. {{ number_format($product->stock_quantity * $product->purchase_price, 2) }}</p>
                    </div>
                    <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg text-center">
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                <td class="px-6 py-4 text-gray-500 dark:text-gray-400">{{ $sale->bill->bill_date->format('M d, Y') }}</td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('bills.show', $sale->bill) }}" class="text-primary-500 hover:underline">
                                        {{ $sale->bill->bill_number }}
                                    </a>
                                </td>
                                <td class="px-6 py-4 text-gray-800 dark:text-white">{{ $sale->bill->customer_name }}</td>
                                <td class="px-6 py-4 text-right text-gray-600 dark:text-gray-400">{{ $sale->quantity }} {{ $product->unit }}</td>
                                <td class="px-6 py-4 text-right text-gray-600 dark:text-gray-400">Rs. {{ number_format($sale->price, 2) }}</td>
                                <td class="px-6 py-4 text-right font-medium text-gray-800 dark:text-white">Rs. {{ number_format($sale->total, 2) }}</td>
                            </tr>
                            @empty
                            <tr>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-medium text-gray-800 dark:text-white">{{ $product->hsn_code }}</span>
                    </div>
                    @endif
                    <div class="flex justify-between items-center">
                        <span class="text-gray-500 dark:text-gray-400">Unit</span>
                        <span class="font-medium text-gray-800 dark:text-white">{{ $product->unit }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-gray-500 dark:text-gray-400">Tax Rate</span>
                        <span class="font-medium text-gray-800 dark:text-white">{{ $product->tax_rate }}%</span>
                    </div>
                    <div class="flex justify-between items-center">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-medium text-gray-800 dark:text-white">{{ number_format($salesSummary['total_quantity']) }} {{ $product->unit }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-medium text-gray-800 dark:text-white">{{ $salesSummary['orders_count'] }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-gray-500 dark:text-gray-400">Avg. Qty/Order</span>
                        <span class="font-medium text-gray-800 dark:text-white">{{ number_format($salesSummary['avg_quantity'], 1) }}</span>
                    </div>
                </div>
            </div>
