    ...
    <div class="container">
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td class="font-bold">{{ $customer->customer_name }}</td>
                    <td>{{ $customer->customer_phone }}</td>
                    <td>{{ $customer->customer_email ?? '-' }}</td>
                    <td class="text-center">{{ $customer->total_orders }}</td>
                    <td class="text-right">Rs. {{ number_format($customer->total_spent, 2) }}</td>
                    <td class="text-right">Rs. {{ number_format($customer->avg_order, 2) }}</td>
                    <td>{{ $customer->last_order ? \Carbon\Carbon::parse($customer->last_order)->format('M d, Y') : '-' }}</td>
                </tr>
                @empty
            ...
            <tfoot>
                <tr class="total-row">
                    ...
                    <td class="text-center font-bold">{{ $customers->sum('total_orders') }}</td>
                    <td class="text-right font-bold">Rs. {{ number_format($customers->sum('total_spent'), 2) }}</td>
                    <td colspan="2"></td>
                </tr>
            </tfoot>
            @endif
