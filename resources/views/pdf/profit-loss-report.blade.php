    ...
    <div class="container">
        ...
        <table class="pl-table">
            <tr>
                ...
                <td class="amount">Rs. {{ number_format($salesRevenue, 2) }}</td>
            </tr>
            <tr>
                <td class="label indent">Tax Collected</td>
                <td class="amount">Rs. {{ number_format($taxCollected, 2) }}</td>
            </tr>
            <tr>
                <td class="label indent">Less: Discounts</td>
                <td class="amount">(Rs. {{ number_format($discounts, 2) }})</td>
            </tr>
            <tr class="subtotal-row">
                <td class="label">Total Revenue</td>
                <td class="amount">Rs. {{ number_format($totalRevenue, 2) }}</td>
            </tr>
        </table>
        ...
        <table class="pl-table">
            <tr>
                ...
                <td class="amount">Rs. {{ number_format($cogs, 2) }}</td>
            </tr>
            <tr class="subtotal-row">
                <td class="label">Gross Profit</td>
                <td class="amount {{ $grossProfit >= 0 ? 'profit-positive' : 'profit-negative' }}">Rs. {{ number_format($grossProfit, 2) }}</td>
            </tr>
        </table>
        ...
        <table class="pl-table">
            ...
            <tr>
                <td class="label indent">{{ $expense->category }}</td>
                <td class="amount">Rs. {{ number_format($expense->total, 2) }}</td>
            </tr>
            @endforeach
            <tr class="subtotal-row">
                <td class="label">Total Operating Expenses</td>
                <td class="amount">Rs. {{ number_format($operatingExpenses, 2) }}</td>
            </tr>
        </table>

        <table class="pl-table" style="margin-top: 15px;">
            <tr class="grand-total-row">
                <td class="label" style="font-size: 13px;">NET {{ $netProfit >= 0 ? 'PROFIT' : 'LOSS' }}</td>
                <td class="amount" style="font-size: 13px;">Rs. {{ number_format(abs($netProfit), 2) }}</td>
            </tr>
        </table>
            ...
            <table class="margins-grid">
                <tr>
                    <td>
                        ...
                        <div class="margin-value {{ $grossMargin >= 0 ? 'profit-positive' : 'profit-negative' }}">{{ number_format($grossMargin, 1) }}%</div>
                    </td>
                    <td>
                        <div class="margin-label">Net Profit Margin</div>
                        <div class="margin-value {{ $netMargin >= 0 ? 'profit-positive' : 'profit-negative' }}">{{ number_format($netMargin, 1) }}%</div>
                    </td>
                </tr>
            </table>
        </div>
