    ...
    <div class="container">
        ...
        <table class="summary-table">
            <tr>
                <td>
                    ...
                    <div class="summary-value">Rs. {{ number_format($summary['outputTax'], 2) }}</div>
                </td>
                <td>
                    <div class="summary-label">Input Tax (Purchases)</div>
                    <div class="summary-value">Rs. {{ number_format($summary['inputTax'], 2) }}</div>
                </td>
                <td>
                    <div class="summary-label">Net Tax Liability</div>
                    <div class="summary-value" style="color: {{ $summary['netTax'] >= 0 ? '#991b1b' : '#166534' }};">
                        Rs. {{ number_format(abs($summary['netTax']), 2) }}
                        ({{ $summary['netTax'] >= 0 ? 'Payable' : 'Credit' }})
                    </div>
                </td>
                <td>
                    <div class="summary-label">Total Taxable Value</div>
                    <div class="summary-value">Rs. {{ number_format($summary['taxableValue'], 2) }}</div>
                </td>
            </tr>
        </table>

        @if(count($outputTaxBreakdown) > 0)
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $rate }}%</td>
                    <td class="text-right">Rs. {{ number_format($data['taxable'], 2) }}</td>
                    <td class="text-right">Rs. {{ number_format($data['cgst'], 2) }}</td>
                    <td class="text-right">Rs. {{ number_format($data['sgst'], 2) }}</td>
                    <td class="text-right">Rs. {{ number_format($data['total'], 2) }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="tax-calc">
            <table style="width: 100%;">
                <tr>
                    ...
                    <td style="text-align: right; padding: 5px 0;">Rs. {{ number_format($summary['outputTax'], 2) }}</td>
                </tr>
                <tr>
                    <td style="padding: 5px 0;">Less: Input Tax Credit (Tax Paid on Purchases)</td>
                    <td style="text-align: right; padding: 5px 0;">(Rs. {{ number_format($summary['inputTax'], 2) }})</td>
                </tr>
                <tr style="border-top: 2px solid #f97316;">
                    <td style="padding: 8px 0; font-weight: bold; font-size: 12px;">Net Tax {{ $summary['netTax'] >= 0 ? 'Payable' : 'Credit' }}</td>
                    <td style="text-align: right; padding: 8px 0; font-weight: bold; font-size: 12px; color: {{ $summary['netTax'] >= 0 ? '#991b1b' : '#166534' }};">
                        Rs. {{ number_format(abs($summary['netTax']), 2) }}
                    </td>
                </tr>
            </table>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                @php $taxAmount = $hsn->taxable_value * ($hsn->tax_rate / 100); @endphp
                <tr>
                    <td>{{ $hsn->hsn_code }}</td>
                    <td>{{ $hsn->description }}</td>
                    <td class="text-right">{{ $hsn->quantity }}</td>
                    <td class="text-right">Rs. {{ number_format($hsn->taxable_value, 2) }}</td>
                    <td class="text-center">{{ $hsn->tax_rate }}%</td>
                    <td class="text-right">Rs. {{ number_format($taxAmount / 2, 2) }}</td>
                    <td class="text-right">Rs. {{ number_format($taxAmount / 2, 2) }}</td>
