    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white">
            ...
            <p class="mt-4 text-sm text-green-100">
                <i class="fas fa-arrow-up mr-1"></i> {{ $revenueGrowth ?? 0 }}% from last period
            </p>
        </div>
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">Rs. {{ number_format($totalExpenses ?? 0, 0) }}</p>
                </div>
                <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center">
            ...
            <p class="mt-4 text-sm text-blue-100">
                <i class="fas fa-arrow-down mr-1"></i> {{ $expenseChange ?? 0 }}% from last period
            </p>
        </div>
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">Rs. {{ number_format($netProfit ?? 0, 0) }}</p>
                </div>
                <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center">
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white">
            ...
            <p class="mt-4 text-sm text-orange-100">
                Avg Order: Rs. {{ number_format($avgOrderValue ?? 0, 0) }}
            </p>
        </div>
    </div>
...
<script>
    ...
    const revenueLabels = {!! json_encode($chartLabels ?? ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']) !!};
    const revenueValues = {!! json_encode($revenueData ?? [0, 0, 0, 0, 0, 0]) !!};
    const expenseValues = {!! json_encode($expenseData ?? [0, 0, 0, 0, 0, 0]) !!};
    ...
    const categoryLabels = {!! json_encode($categoryLabels ?? ['Category 1', 'Category 2', 'Category 3']) !!};
    const categoryValues = {!! json_encode($categoryData ?? [30, 40, 30]) !!};
