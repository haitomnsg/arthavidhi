...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center space-x-3">
            ...
            <a href="{{ route('reports.tax', ['start_date' => request('start_date'), 'end_date' => request('end_date'), 'export' => 'csv']) }}" 
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-500">{{ $summary['net_tax'] >= 0 ? 'Payable' : 'Credit' }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800">Rs. {{ number_format($summary['taxable_value'], 2) }}</p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr>
                            <td class="px-6 py-4 font-medium text-gray-800">{{ $rate }}%</td>
                            <td class="px-6 py-4 text-right text-gray-600">Rs. {{ number_format($data['taxable'], 2) }}</td>
                            <td class="px-6 py-4 text-right text-gray-600">Rs. {{ number_format($data['cgst'], 2) }}</td>
                            <td class="px-6 py-4 text-right text-gray-600">Rs. {{ number_format($data['sgst'], 2) }}</td>
                            <td class="px-6 py-4 text-right font-medium text-gray-800">Rs. {{ number_format($data['total'], 2) }}</td>
                        </tr>
                        @endforeach
                    </tbody>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-4 max-w-md">
            ...
            <div class="flex justify-between items-center py-4 {{ $summary['net_tax'] >= 0 ? 'bg-orange-50' : 'bg-blue-50' }} -mx-6 px-6 rounded-lg">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 font-medium text-gray-800">{{ $hsn->hsn_code ?: 'N/A' }}</td>
                        <td class="px-6 py-4 text-gray-600">{{ Str::limit($hsn->description, 30) }}</td>
                        <td class="px-6 py-4 text-right text-gray-600">{{ number_format($hsn->quantity, 2) }}</td>
                        <td class="px-6 py-4 text-right text-gray-800">Rs. {{ number_format($hsn->taxable_value, 2) }}</td>
                        <td class="px-6 py-4 text-right text-gray-600">{{ $hsn->tax_rate }}%</td>
                        <td class="px-6 py-4 text-right text-gray-600">Rs. {{ number_format($hsn->taxable_value * $hsn->tax_rate / 200, 2) }}</td>
                        <td class="px-6 py-4 text-right text-gray-600">Rs. {{ number_format($hsn->taxable_value * $hsn->tax_rate / 200, 2) }}</td>
                        <td class="px-6 py-4 text-right font-medium text-gray-800">Rs. {{ number_format($hsn->taxable_value + ($hsn->taxable_value * $hsn->tax_rate / 100), 2) }}</td>
                    </tr>
                    @empty
                    <tr>
