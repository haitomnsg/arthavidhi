...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center space-x-4">
            <a href="{{ route('purchases.index') }}" class="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                <i class="fas fa-arrow-left text-gray-600 dark:text-gray-400"></i>
            </a>
            <div>
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Purchase #{{ $purchase->purchase_number }}</h1>
                <p class="text-gray-500 dark:text-gray-400">{{ $purchase->purchase_date->format('F d, Y') }}</p>
            </div>
        </div>
        <div class="flex items-center space-x-3">
            <span class="px-3 py-1 text-sm font-medium rounded-full 
                {{ $purchase->status === 'received' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : '' }}
            ...
            <span class="px-3 py-1 text-sm font-medium rounded-full 
                {{ $purchase->payment_status === 'paid' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : '' }}
                {{ $purchase->payment_status === 'partial' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400' : '' }}
                {{ $purchase->payment_status === 'unpaid' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' : '' }}">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <p class="font-bold text-gray-800 dark:text-white text-lg">{{ $purchase->supplier_name }}</p>
                        @if($purchase->supplier_contact)
                        <p class="text-gray-600 dark:text-gray-400"><i class="fas fa-user mr-2 text-gray-400"></i>{{ $purchase->supplier_contact }}</p>
                        @endif
                        @if($purchase->supplier_phone)
                        <p class="text-gray-600 dark:text-gray-400"><i class="fas fa-phone mr-2 text-gray-400"></i>{{ $purchase->supplier_phone }}</p>
                        @endif
                        @if($purchase->supplier_email)
                        <p class="text-gray-600 dark:text-gray-400"><i class="fas fa-envelope mr-2 text-gray-400"></i>{{ $purchase->supplier_email }}</p>
                        @endif
                    </div>
                    <div class="space-y-2">
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 text-right text-gray-800 dark:text-white">Rs. {{ number_format($item->cost_price, 2) }}</td>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <p class="text-gray-600 dark:text-gray-400 whitespace-pre-line">{{ $purchase->notes }}</p>
            </div>
            @endif
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-800 dark:text-white">{{ $purchase->purchase_number }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Date</span>
                        <span class="font-medium text-gray-800 dark:text-white">{{ $purchase->purchase_date->format('M d, Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-800 dark:text-white">{{ $purchase->items->count() }}</span>
                    </div>
                </div>
            </div>
